defmodule Issues.TableFormatterTest do
  use ExUnit.Case
  import ExUnit.CaptureIO

  alias Issues.TableFormatter, as: Formatter

  def simple_test_data do
    [[c1: "r1 c1", c2: "r1 c2",  c3: "r1 c3", c4: "r1+++c4"],
     [c1: "r2 c1", c2: "r2 c2",  c3: "r2 c3", c4: "r2 c4"  ],
     [c1: "r3 c1", c2: "r3 c2",  c3: "r3 c3", c4: "r3 c4"  ],
     [c1: "r4 c1", c2: "r4++c2", c3: "r4 c3", c4: "r4 c4"  ]]
  end

  def headers, do: [:c1, :c2, :c4]

  def split_with_three_columns do
    Formatter.split_into_columns(simple_test_data, headers)
  end

  describe "split_into_columns/2" do
    test "split_into_columns" do
      columns = split_with_three_columns
      assert length(columns)     == length(headers)
      assert List.first(columns) == ["r1 c1", "r2 c1", "r3 c1", "r4 c1"]
      assert List.last(columns)  == ["r1+++c4", "r2 c4", "r3 c4", "r4 c4"]
    end
  end

  describe "widths_of/1" do
    test "column_widths" do
      widths = Formatter.widths_of(split_with_three_columns)
      assert widths == [5, 6, 7]
    end
  end

  describe "widths_of/1" do
    test "correct format string returned" do
      assert Formatter.format_for([9, 10, 11]) == "~-9s | ~-10s | ~-11s~n"
    end
  end

  describe "print_table_for_columns/1" do
    test "Output is correct" do
      result = capture_io fn ->
        Formatter.print_table_for_columns(simple_test_data, headers)
      end
      assert result == """
      c1    | c2     | c4\s\s\s\s\s
      ------+--------+--------
      r1 c1 | r1 c2  | r1+++c4
      r2 c1 | r2 c2  | r2 c4\s\s
      r3 c1 | r3 c2  | r3 c4\s\s
      r4 c1 | r4++c2 | r4 c4\s\s
      """
    end

    test "column widths handle wider headers" do
      result = capture_io fn ->
        Formatter.print_table_for_columns(
          [[wide_header: "value", also_wider: "value"]],
          [:wide_header, :also_wider]
        )
      end
      assert result == """
      wide_header | also_wider
      ------------+-----------
      value       | value\s\s\s\s\s
      """
    end
  end
end
